...
const VARIANT_STYLES = {
  featured: {
    article: 'overflow-hidden rounded-sm bg-white shadow-sm p-4 md:p-5 group cursor-pointer transition-all duration-300 hover:shadow-md border border-neutral-100/50',
    title: 'text-base md:text-lg font-serif font-medium text-[#201c18] leading-snug truncate group-hover:text-[#7a6050] transition-colors',
    category: 'mt-0.5 text-xs font-raleway text-[#606060] tracking-wide',
    metaRow: 'flex items-center justify-between text-sm text-[#6d6357] mt-3 pt-3 border-t border-dashed border-[#e8e0d8]',
    location: 'inline-flex items-center gap-1 text-[#5a4a3a] font-raleway font-medium truncate max-w-[60%]',
    price: 'font-serif font-bold text-[#3a2a1a] text-base md:text-lg shrink-0',
    rating: 'inline-flex items-center gap-0.5 text-sm font-medium text-[#544d43]',
  },
...
const VendorCard = memo(({ vendor, variant = 'grid', onClick }) => {
  ...
  const imageSrc = vendor?.thumbnailImage || vendor?.image || '/dummy-image-square.jpg';
  ...
  const displayPrice = useMemo(() => {
    if (vendor?.packagePriceRange?.low !== undefined && vendor?.packagePriceRange?.high !== undefined) {
      ...
      <div className='pt-4'>
        <div className='flex items-start justify-between gap-2 mb-1'>
          <div className='min-w-0 flex-1'>
            <h3 className={styles.title}>{vendor?.businessName || vendor?.name}</h3>
            <p className={styles.category}>{vendor?.category || 'Wedding Professional'}</p>
          </div>
