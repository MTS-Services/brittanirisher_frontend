...
const CoupledashboardApi = apiSlice.injectEndpoints({
  endpoints: (builder) => ({
    ...
    getVendorSuggested: builder.query({
      query: (params = {}) => {
        ...
        if (minPrice !== undefined && minPrice !== '')
          queryObj.minPrice = String(minPrice);
        if (maxPrice !== undefined && maxPrice !== '')
          queryObj.maxPrice = String(maxPrice);
      ...
      serializeQueryArgs: ({ queryArgs }) => {
        const resolved =
          typeof queryArgs === 'number' ? { page: queryArgs } : queryArgs;
    ...
    createCoupleExpense: builder.mutation({
      ...
      invalidatesTags: [{ type: 'CoupleExpense', id: 'LIST' }, 'Dashboard'],
    }),
        ...
        try {
          ...
          dispatch(
            CoupledashboardApi.util.updateQueryData(
              ...
              (draft) => {
                const vendorList =
                  draft?.data?.vendors || draft?.vendors || draft;
