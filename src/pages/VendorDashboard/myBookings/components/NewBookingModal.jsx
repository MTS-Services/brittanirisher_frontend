  ...
  <ModalShell onClose={onClose} widthClass='max-w-lg'>
    ...
    <form className='space-y-4 px-5 py-5' onSubmit={onSubmit}>
      {[
        ...
        { key: 'price', label: 'Price', placeholder: '75000.50', type: 'number' },
      ].map((field) => (
      ...
      <div className='grid grid-cols-1 gap-4'>
        <label className='block'>
          ...
          <p className='mt-1 text-xs text-[#7d8794]'>Dropdown shows only package name; the selected id is sent to the API.</p>
        </label>
      </div>
      ...
      <div className='flex items-center justify-end gap-3 pt-2'>
        ...
        <button type='submit' disabled={isSubmitting} className='h-10 rounded-sm bg-[#6f7969] px-6 text-[14px] text-white disabled:opacity-60'>
