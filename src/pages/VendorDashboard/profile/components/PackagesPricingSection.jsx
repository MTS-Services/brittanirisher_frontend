...
───────────────────────────────────────── */
const PackageModal = ({ editPackageId, onClose, refetch }) => {
  ...
  const [createVendorPackage, { isLoading: isCreating }] =
    useCreateVendorPackageMutation();
  const [updateVendorPackage, { isLoading: isUpdating }] =
    useUpdateVendorPackageMutation();
          ...
          {isEditMode && isFetching ? (
            ...
            <div className='space-y-4 px-5 py-4'>
              ...
              <div>
                ...
                >
                  Features
                  <span className='ml-1.5 text-xs text-[#989898]'>
                    (one per line)
                  </span>
                </label>
                <textarea
...
───────────────────────────────────────── */
const PackagesPricingSection = ({ packages, refetch }) => {
  const [modalState, setModalState] = useState({ open: false, editId: null });
  const [deleteConfirm, setDeleteConfirm] = useState({ open: false, id: null });

  const [deleteVendorPackage, { isLoading: isDeleting }] =
    useDeleteVendorPackageMutation();
