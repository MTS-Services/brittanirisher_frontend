...
const ChecklistSectionCard = ({
  ...
  const handleTaskChange = async (taskId, currentStatus) => {
    ...
    const nextCompletedCount = currentStatus
      ? completedCount - 1
      : completedCount + 1;
    ...
    <div className='bg-white border border-[#EBEBEB] rounded-2xl p-6 space-y-6'>
      <div className='flex justify-between items-start'>
        ...
        <div className='text-right flex items-center gap-3'>
          <div>
            <span className='block text-[10px] font-bold text-[#b2b2b2] tracking-wider uppercase'>
              COMPLETION
            </span>
      ...
      <div className='space-y-3'>
        {tasks.map((task) => (
          <div
            key={task.id}
            className='flex flex-col rounded-xl border border-[#EBEBEB] bg-[#FDFCFC] p-4 space-y-2 transition hover:bg-gray-50'
          >
            <label className='flex cursor-pointer md:items-center flex-col md:flex-row justify-between select-none'>
              ...
              <div className='flex items-center gap-2'>
                ...
                {task.priority && (
                  <span
                    className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${getPriorityStyle(task.priority)}`}
                  >
                    {task.priority}
                  </span>
                )}
      ...
      <div className='space-y-2 pt-2 border-t border-gray-100'>
        <label className='text-xs font-bold text-[#707070] uppercase tracking-wider'>
          PHASE NOTES
        </label>
        <textarea
          value={noteText}
