...
const FAQ_ITEMS = [
  {
    ...
    body:
      'Nibh quisque suscipit fermentum netus nulla cras porttitor euismod nulla. Orci, dictumst nec aliquet id ullamcorper venenatis. Fermentum nulla cras porttitor euismod nulla.',
   
  },
  {
    title: 'How do I list my services on this platform?',
    body:
      'Getting started is simple and straightforward. Create your vendor account, complete your profile with your services and portfolio, set your pricing, and you\'ll be visible to couples immediately. Our team is here to help you every step of the way.',
  },
  {
    title: 'Is there a fee for registration?',
    body:
      'Registration is completely free! We only charge subscription fees for vendors who want access to our premium features and lead generation system. Choose the plan that works best for your business.',
  },
  {
    title: 'How do couples contact me?',
    body:
      'Couples can reach out to you directly through your profile page, via phone, email, or our built-in messaging system. You\'ll receive notifications for every inquiry so you never miss a potential booking.',
  },
  {
    title: 'Can I update my portfolio after publishing?',
    body:
      'Absolutely! You can update your portfolio, services, pricing, and availability anytime through your vendor dashboard. Changes go live immediately to keep your profile current and fresh.',
  },
  {
    title: 'How does the booking system work?',
    body:
      'After a couple expresses interest, you can communicate directly to discuss availability, pricing, and requirements. Once you\'ve agreed on terms, you can finalize the booking through our secure system.',
  },
  {
    title: 'What are the benefits of a "Verified Vendor" badge?',
    body:
      'Our Verified Vendor badge increases couples\' trust and confidence in your services. Verified vendors appear higher in search results and receive priority placement on the platform, leading to more inquiries and bookings.',
  },
];
...
const PricingPage = memo(() => {
  useSEO({
    title: 'Pricing',
    description: 'Review vendor pricing tiers, compare plans, and find the right package for your business.',
    ...
    <div className='overflow-hidden bg-[#f4f0ea] text-[#2d2d2d]'>
        ...
        <div className='absolute inset-0 flex items-center justify-center px-4 text-center text-white'>
          <div className='space-y-2' style={{ maxWidth: 781 }}>
            ...
            <p className='font-raleway text-[20px] leading-8'>
              Vow &amp; Vendor connects you with thousands of local brides actively looking for their perfect wedding team.
            </p>
          </div>
        </div>
      </section>
      ...
      <section className='relative px-4 sm:px-6 overflow-hidden'>
        <img
          ...
          alt=''
          aria-hidden='true'
          className='pointer-events-none absolute hidden lg:block -right-2 -bottom-15 w-[320px] h-auto z-50'
        
        />
        
        <div className='mx-auto container px-4 sm:px-6 lg:px-8 py-14 sm:py-20 relative z-10'>
          <div className='mx-auto max-w-4/6 text-center '>
            <h2 className='font-playfair text-3xl md:text-4xl lg:text-5xl leading-none text-[#090909]'>Vendor Pricing</h2>
            <p className='mt-6 font-raleway text-[20px] leading-8 text-[#857f7a]'>
              Choose the perfect plan to showcase your services, connect with brides, and grow your wedding business with Vow &amp; Vendor.
            </p>
          </div>
      ...
      <section className='relative overflow-hidden   bg-[#d7dfd6] '>
        ...
        <div className='mx-auto   max-w-7xl px-4 py-14 sm:px-6 lg:px-8 lg:py-20'>
          <h2 className='text-center font-playfair  text-3xl md:text-4xl lg:text-5xl leading-none text-[#090909]'>Compare Feature Plans</h2>

          <div className='mt-12 overflow-hidden border border-[#f7f5f1] bg-transparent'>
            <div className='grid grid-cols-[1.3fr_1fr_1fr_1fr] border-b border-[#f7f5f1] px-8 py-3.5 font-raleway text-lg md:text-xl font-medium text-[#070707] lg:px-10'>
              <div>Features</div>
              <div>Starter</div>
            ...
            {FEATURE_ROWS.map(([feature, starter, pro, premium]) => (
              <div key={feature} className='grid grid-cols-[1.3fr_1fr_1fr_1fr] border-b border-[#f7f5f1] px-8 py-5 font-raleway text-[20px] font-medium text-[#070707] lg:px-10'>
                <div>{feature}</div>
                <div className='flex items-center justify-start'>{starter === 'tick' ? <Check size={24} className='text-[#16B21B]' /> : starter === 'cross' ? <X size={24} className='text-[#6b6b6b]' /> : starter}</div>
                <div className='flex items-center justify-start'>{pro === 'tick' ? <Check size={24} className='text-[#16B21B]' /> : pro === 'cross' ? <X size={24} className='text-[#6b6b6b]' /> : pro}</div>
                <div className='flex items-center justify-start'>{premium === 'tick' ? <Check size={24} className='text-[#16B21B]' /> : premium === 'cross' ? <X size={24} className='text-[#6b6b6b]' /> : premium}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className='relative px-4 py-14 sm:px-6 lg:px-8 lg:py-20'>
        ...
        <div className='mx-auto max-w-6xl'>
          <div className='mx-auto text-center '>
            <h2 className='font-playfair text-[48px] leading-none text-[#2d2d2d]'>Vendor FAQs</h2>
            <p className='mt-4 font-raleway text-[20px] leading-7 text-[#857f7a]'>Quick answers to common questions</p>
          </div>

          <div className='mt-12 space-y-3'>
            {FAQ_ITEMS.map((item) => (
              <details key={item.title} open={item.open} className='group bg-[#f0e9e1] px-6 py-4'>
          ...
          <div className='mx-auto mt-20 rounded-2xl bg-[#5c665b] px-6 py-16 text-center text-white lg:px-16 lg:py-20' style={{ maxWidth: 1538 }}>
            <h3 className='font-playfair text-[48px] leading-none'>Become a Vendor Today</h3>
            <p className='mx-auto mt-6 font-raleway text-[20px] leading-8' style={{ maxWidth: 633 }}>
              Don&apos;t miss out on the busiest wedding season yet. Secure your spot in our directory and start receiving leads.
            </p>
            <Link
              to={ROUTES.LOGIN}
